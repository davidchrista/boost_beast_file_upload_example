                    ...
                    http::request<Body, http::basic_fields<Allocator>> &&req, Send &&send)
{
  ...
  if (req.target().empty() || req.target()[0] != '/' ||
    ...
    return send(bad_request("Illegal request-target: " + req.target().to_string()));
  ...
  {
    if (req.body().size() <= 0)
      return send(bad_request("No file"));
...
int main(int argc, char *argv[])
{
  ...
  auto const port = static_cast<unsigned short>(std::atoi("3001"));
